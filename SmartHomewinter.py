...
pv_winter_output=[0,0,0,0,0,0,0,300,600,1500,2000,2500,3000,2500,2000,1500,550,300,0,0,0,0,0,0]
...
while True:   
    ...
    for x in range(7,18):
        if pv_winter_output[x] < (peak_power/10):
            battery_soc+=pv_winter_output[x]
...
print(price)
sumprice=sum(price)
print(f"Daily energy price with/out load shifting in winter is {sumprice}₺")
...
ax.set_ylim(ymin=0)
ax.set_xlim(xmin=0)
ax.set_title("Daily Based/Shifted Loading For Winter")
plt.xlabel('Hours')
plt.ylabel("Power Level(W)")
plt.xticks()
plt.tight_layout()
...
ax.set_ylim(ymin=0)
ax.set_xlim(xmin=0)
ax.set_title("Daily energy prices using triple-time tariff with/out load shifting in winter")
plt.xlabel('Hours')
plt.ylabel("Price(₺)")
plt.xticks()
plt.tight_layout()
